...
// Verify dx=0.5 fix works for all problematic presets
...
for (const preset of PRESETS) {
    ...
    function stepCPU() {
        ...
        for (let y = 0; y < gridSize; y++) {
            for (let x = 0; x < gridSize; x++) {
                ...
                const newA = a + (Du * laplaceA - reaction + preset.f * (1 - a)) * timestep;
                const newB = b + (Dv * laplaceB + reaction - (preset.k + preset.f) * b) * timestep;
    ...
    const status = avgB > 0.002 ? '✓ WORKS' : '✗ DIES';
    console.log('%s: f=%s, k=%s → B avg=%s %s', preset.name.padEnd(25), preset.f, preset.k, avgB.toFixed(6), status);
}
