...
// Test with Random Noise to see if it turns red
...
// Random noise initialization (current implementation)
for (let i = 0; i < size; i++) {
    if (Math.random() < 0.02) {  // 2% of cells
        gridB[i] = 0.05 + Math.random() * 0.1;  // 0.05 to 0.15
    }
}

console.log('=== INITIAL STATE (RANDOM NOISE) ===');
